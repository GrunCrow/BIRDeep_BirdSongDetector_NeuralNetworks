...
'''
File based on the code done by Justin Kay:
...
# Function to process each image and save predictions
def process_and_save_predictions(image_path, model, save_dir, softmax_temperature_value):
    # Run prediction on the image
    boxes = run_predict(image_path, model, softmax_temperature_value=softmax_temperature_value)
    
    # Generate the output path by replacing the image extension with .txt
    # and ensuring the directory structure is replicated in save_dir
    output_path = os.path.join(save_dir, os.path.splitext(os.path.relpath(image_path, start=input_dir))[0] + '.txt')
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    
    # Open the output file and write predictions
    with open(output_path, 'w') as f:
        for box in boxes:
            # Format: class, bbox coordinates (x0, y0, x1, y1), score, logit_score
            line = f"{box['best_cls']}, {box['bbox'][0]}, {box['bbox'][1]}, {box['bbox'][2]}, {box['bbox'][3]}, {box['best_conf']}, {max(box['logits'])}\n"
            f.write(line)
...
def run_predict(input_path, model_path, score_threshold=0.5, iou_threshold=0.7, save_image = False, save_json = False, category_mapping = None, softmax_temperature_value = 1.0, agnostic = False):
    ...
    for img_path in img_paths:
        ...
        # Read the input file to get image paths
        with open(input_txt_path) as f:
            image_paths = f.read().splitlines()
        
        # Process each image
        for image_path in image_paths:
            process_and_save_predictions(image_path, model, save_dir, softmax_temperature_value)
...
def main():
    ...
    # Adapt as project structure
    experiment_name = "12_AllBG_LessESC50_FF_Small"
    PROJECT_MODEL_PATH = f"../BIRDeep/{experiment_name}/weights/best.pt"
    ...
    # If TXT file with image paths -> with YOLO TXT expected structure
    input_path = "../Data/Dataset/TXTs/val.txt"

    # run inference, other parameters: save_image = False, save_json = False, category_mapping = None, softmax_temperature_value = 1.0, agnostic = False
    results = run_predict(input_path = input_path, model_path = model_path, score_threshold=threshold, iou_threshold=nms_threshold)

    print("Processed", len(results), "boxes")
    print("The first one is", results[0])

    if SAVE_TEST_IMG:
        plot_image(input_path, results)
